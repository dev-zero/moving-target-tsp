...
public:
    ...
    /*
     * calculate the travelling time for all permutations (exhaustive search)
     * Warning: O(n!)
     * returns an array containing permutated indices to the targets
     * use for_each_permutated_target() to get the targets
     */

    std::pair<std::shared_ptr<std::vector<unsigned long>>, std::shared_ptr<std::vector<double>>> shortest_path_exhaustive_search(double v) const
    {
        ...
        std::shared_ptr<std::vector<unsigned long>> shortest_path_permutation(new std::vector<unsigned long>); // holding the permutation with the shortest path
        std::shared_ptr<std::vector<double>> shortest_path_times(new std::vector<double>(size()));
        std::vector<double> path_times(size()); // stores the corresponding times (answer to: at which 't' did we hit the target?)
        ...
        std::vector<unsigned long> permutation(size());
        ...
        do
        {
            ...
            std::cout << get_target(permutation.front()).name << " (" << total_t << ")";
            path_times.front() = total_t;

            for (auto j(permutation.begin()), j_end(permutation.end()-1); j != j_end; ++j)
            {
                total_t += calculate_time(v,
                        get_target(*j).position + total_t*get_target(*j).velocity,
                        get_target(*(j+1)).position + total_t*get_target(*(j+1)).velocity,
                        get_target(*(j+1)).velocity);
                std::cout << " -> " << get_target(*(j+1)).name << " (" << total_t << ")";
        ...
        return std::make_pair(shortest_path_permutation, shortest_path_times);
    }
};
...
int main(int argc, char* argv[])
{
    ...
    double smallest_start_distance(std::numeric_limits<double>::infinity()); // the smallest distance between two targets at t=0, needed to calculated an appropriate radius for the 3D-objects
    ...
    std::pair<std::shared_ptr<std::vector<unsigned long>>, std::shared_ptr<std::vector<double>>> shortest_path(app.shortest_path_exhaustive_search(v));
    /*
     * check whether we really found a valid tour
     */
    if (!shortest_path.first->size())
    {
        std::cout << "no valid path found, try with higher velocity" << std::endl;
        return 1;
    }
     ...
     */
    std::cout << "shortest path was: " << app.get_target(shortest_path.first->front()).name << " (" << shortest_path.second->front() << ")";
    for (auto i(shortest_path.first->begin()), i_end(shortest_path.first->end()-1), i_begin(shortest_path.first->begin()); i != i_end; ++i)
    {
        std::cout << " -> " << app.get_target(*(i+1)).name << " (" << shortest_path.second->at(std::distance(i_begin, i)+1) << ")";
    }
    std::cout << std::endl;
    ...
    app.for_each_permutated_target(
            [&shortest_path, &target_positions_at_hit, &addStar, &color, smallest_start_distance](const Target& t, size_t n) {
            osg::Vec3 position(
                t.position(0) + shortest_path.second->at(n)*t.velocity(0),
                t.position(1) + shortest_path.second->at(n)*t.velocity(1),
                t.position(2) + shortest_path.second->at(n)*t.velocity(2));
            addStar(position[0], position[1], position[2], smallest_start_distance/2.0, color);
            target_positions_at_hit->push_back(position);
            },
            [&shortest_path](size_t n) {
            return (*shortest_path.first)[n];
            }
            );
    ...
    widget->view->setSceneData(root.get());
